    ...
    function(exports) {
          ...
          if (url || api) {
            ...
            self.request({
              ...
              success: function(res) {
                templateData = data
                renderTemplate(tem, res[conf.response.dataName])//res['data']
      ...
      self.createRequestParams = function(params) {
        ...
        var defaultParams = {
          ...
          success: function(res) {
            var status = res[conf.response.statusName]//res['code']
            if (status != conf.response.statusCode.ok) {// !=0
              if (status == conf.response.statusCode.logout) {//401
                layui.admin.logout()
              } else {
                self.log(
                  '返回状态码异常：' +
          ...
          error: function(res) {
           ...
           /* if (res[conf.response.statusName] == conf.logoutHttpCode) {
              layui.admin.logout()
            } */
            else {
              layer.msg('请求超时！')
